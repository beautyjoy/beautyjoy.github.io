...
#
# `data-a11y-external-errors` marks a third-party embed (Stripe's buy button,
# Facebook's share widget) whose markup is generated by the vendor inside their
# own iframe. We can't fix their DOM, and excluding the wrapper also keeps axe
# out of the iframe, so these no longer fail intermittently as the widget loads.
excluded_elements = [
